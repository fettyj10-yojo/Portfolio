...
export default function ParticleName({
  ...
  useEffect(() => {
    ...
    const createParticles = () => {
      ...
      // Use the same lightweight rendering profile at every breakpoint. The
      // previous desktop-only profile produced many more particles and made
      // the name assemble much more slowly than it does on mobile.
      const ratio = 1;
      ...
      for (let y = 0; y < height; y += step) {
        for (let x = 0; x < width; x += step) {
          ...
          const letter = letterRanges.find(
          ...
          const letterProgress = letter
            ? Math.max(
                ...
                Math.min(
                  1,
                  (x - letter.start) / Math.max(1, letter.end - letter.start),
                ),
              )
            : 0;
          const releaseAt =
    ...
    const draw = (time = 0) => {
      ...
      for (const particle of particles) {
        if (reduceMotion) {
          ...
          particle.vx =
            (particle.vx + (particle.tx - particle.x) * 0.016) * 0.9;
          particle.vy =
            (particle.vy + (particle.ty + shimmer - particle.y) * 0.016) * 0.9;
        ...
        const reveal = reduceMotion
          ? 1
          : Math.min(1, (elapsed - particle.releaseAt) / 450);
        context.globalAlpha =
          reveal * (0.72 + Math.sin(time * 0.002 + particle.phase) * 0.2);
      ...
      if (
        !hasCompleted &&
        (reduceMotion || elapsed >= assemblyCompleteAt + 1200)
      ) {
        hasCompleted = true;
        onComplete?.();
      }
